...
@click.option('-case_id',
    ...
    nargs=1,
    required=False,
    help='load all affected patients from a scout case'
)
...
@click.option('-suspects_only',
    is_flag=True,
    default=False,
    help='share only pinned variants from a case'
)

@click.pass_context
def mme_patient(context, email, json_file, case_id, token, mme_url, genes_only, suspects_only):
    """Share one or more patients to MatchMaker Exchange
        ...
        Args:
            ...
            mme_url(str) : URL of the matchmaker instance the patients should be saved to (default is localhost)

            token(str): authorization token recognized by MME server

            genes_only(bool flag): if True only gene names will be part of genomic features of patients, not variants

            suspects_only(bool flag): if True only pinned variants from a case will be shared to MatchMaker
    ...
    contact_info = {
        'name' : user_obj['name'],
        'href' : user_obj['email'],
        'institution' : 'User of Scout variant browser. Clinical Genomics, SciLifeLab Stockholm, Sweden.'
    }
    ...
    #        if not individual['phenotype'] == 2: # affected
    #            continue
    #        mme_patient = {}
    ...
    if not mme_patient_list:
        LOG.warning('No patients to add to MatchMaker.')
        context.abort()
